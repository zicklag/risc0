...
risc0_receipt* risc0_receipt_from_raw(risc0_error* err, const uint8_t* journal_raw, size_t journal_len, const uint32_t* seal_raw, size_t seal_len) {
  return ffi_wrap<risc0_receipt*>(err, nullptr, [&] {
    risc0::Buffer journal(journal_raw, journal_raw + journal_len);
    risc0::BufferU32 seal(seal_raw, seal_raw + seal_len);
    risc0::Receipt receipt(seal, journal);
    return new risc0_receipt{receipt};
  });
}
